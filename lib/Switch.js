...
class Switch extends React.Component {
  ...
  handleMouseUp () {
    ...
    if (this.state.dragging) {
      const checked = this.getOffset() - this.getMinOffset() > (this.getMaxOffset() - this.getMinOffset()) / 2
      this.setState({
        dragging: false
      })
      this.recordChange(checked)
    } else {
      this.recordChange(!this.props.checked)
    }
  }

  recordChange (checked) {
